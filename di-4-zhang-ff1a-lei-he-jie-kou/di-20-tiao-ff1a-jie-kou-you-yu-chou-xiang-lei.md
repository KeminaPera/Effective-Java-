...
**现有类可以很容易地被改造以实现一个新的接口。**你必须做的事情就是添加所需的方法，假如这些方法还没有编写的话，同时在类的声明处添加实现说明。例如，但JDK里添加了Comparable接口，Iterable接口，以及Autocloseable接口时，很多现有类都被改造并实现了它们。通常情况下，现有类无法被改造以扩展一个新的抽象类。如果你想让两个类去扩展一个相同的抽象类
